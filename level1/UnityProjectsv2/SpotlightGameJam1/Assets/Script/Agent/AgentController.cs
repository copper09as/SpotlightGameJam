...
public override void OnActionReceived(ActionBuffers actions)
{
    ...
    private void RewardObstacleJump(float moveX, bool jumpPressed)
    {
        ...
        bool beforeGoal = goalTransform == null || transform.position.x < goalTransform.position.x - 0.75f;

        if (beforeGoal)
        {
            AddReward(jumpPressed ? 0.01f : -0.003f);
        }

        if (grounded && obstacleAhead)
        {
            AddReward(jumpPressed ? 0.04f : -0.006f);
        }
        else if (grounded && almostStopped)
        {
            AddReward(jumpPressed ? 0.01f : -0.002f);
        }
    }
    ...
    private bool IsStompFirstMode()
    {
        ...
        return behaviorParameters != null
            ...
            && behaviorParameters.Model.name.IndexOf("Stomp", StringComparison.OrdinalIgnoreCase) >= 0;
    }
