...
public static class MLAgentsTrainingAutomation
{
    ...
    public static void BuildFromCommandFile()
    {
        try
        {
            ...
            BuildScene(command.scenePath, command.outputPath, command.stompFirst, command.inference, command.modelPath);
    ...
    private static void BuildScene(string scenePath, string outputPath, bool stompFirst = false, bool inference = false, string modelPath = null)
    {
        ...
        ConfigureTrainingScene(stompFirst, inference, modelPath);
    ...
    private static void ConfigureTrainingScene(bool stompFirst, bool inference, string modelPath)
    {
        ...
        if (inference)
        {
            ...
            {
                throw new ArgumentException("Inference build requires a modelPath.");
            }

            model = AssetDatabase.LoadAssetAtPath<Unity.Barracuda.NNModel>(modelPath);
            if (model == null)
            {
        ...
        foreach (var behavior in UnityEngine.Object.FindObjectsOfType<BehaviorParameters>(true))
        {
            ...
            behavior.BehaviorType = inference ? BehaviorType.InferenceOnly : BehaviorType.Default;
